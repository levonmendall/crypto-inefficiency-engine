...
#cards .lane-history-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-bottom:8px}
...
function renderLaneHistoricalEvidence(cards,p){
  ...
  for(const c of cards){
    ...
    const node=document.querySelector(`#cards .card[data-mechanism-id="${CSS.escape(lane)}"]`);
    if(!node)continue;
    ...
    const rawTotal=data.funnels.reduce((sum,row)=>sum+(Number.isFinite(+row?.funnel?.raw_candidate_count)?+row.funnel.raw_candidate_count:0),0);
    const emittedTotal=data.funnels.reduce((sum,row)=>sum+(Number.isFinite(+row?.funnel?.emitted_candidate_count)?+row.funnel.emitted_candidate_count:0),0);
    const bestNetValues=data.funnels.map(row=>+row?.funnel?.best_net_economics).filter(Number.isFinite);
    const bestNet=bestNetValues.length?Math.max(...bestNetValues):null;
    const recentFunnels=[...data.funnels].sort((a,b)=>new Date(b.item?.observed_at||0)-new Date(a.item?.observed_at||0)).slice(0,3);
    const recentCandidates=[...data.selected].sort((a,b)=>new Date(b.observed_at||b?.candidate?.observed_at||0)-new Date(a.observed_at||a?.candidate?.observed_at||0)).slice(0,3);
    const state=p?.complete?'complete':p?.runtime?.detail?.waiting_for_live_observatory_boundary?'caught up / waiting':'running';
    host.innerHTML=`<div class="lane-history-head"><div><div class="lane-history-title">Historical opportunity evidence</div><div class="lane-history-summary">Aug. 21 → live boundary · diagnostic only</div></div><span class="badge">${esc(state.toUpperCase())}</span></div><div class="lane-history-grid"><div class="lane-history-stat"><div class="k">Historical raw</div><div class="v">${num(rawTotal)}</div></div><div class="lane-history-stat"><div class="k">Historical emitted</div><div class="v">${num(emittedTotal)}</div></div><div class="lane-history-stat"><div class="k">Best historical net</div><div class="v">${bestNet===null?'—':laneHistoryPct(bestNet)}</div></div></div>${recentCandidates.map(laneHistoryCandidateRow).join('')}${recentFunnels.map(laneHistoryFunnelRow).join('')}${(!recentCandidates.length&&!recentFunnels.length)?'<div class="lane-history-unassigned">No persisted historical evidence has been mapped to this lane yet.</div>':''}`;
  }
