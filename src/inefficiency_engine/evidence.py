        ...
        with self._connect() as db:
            db.execute(
                ...
                (
                    ...
                    _utc_now().isoformat(),
                    json.dumps(analysis_config or {}, sort_keys=True),
                ),
            )
            ...
            for quote in funding_quotes:
                ...
                db.execute(
                    "INSERT INTO funding_quotes(scan_id, venue, asset, observed_at, payload_json, lineage_hash) VALUES (?, ?, ?, ?, ?, ?)",
                    (scan_id, quote.venue, quote.asset, quote.observed_at.isoformat(), payload, hashlib.sha256(payload.encode()).hexdigest()),
                )
            for quote in market_quotes:
                payload = _canonical_json(quote)
                db.execute(
                    "INSERT INTO market_quotes(scan_id, venue, asset, observed_at, payload_json, lineage_hash) VALUES (?, ?, ?, ?, ?, ?)",
                    (scan_id, quote.venue, quote.asset, quote.observed_at.isoformat(), payload, hashlib.sha256(payload.encode()).hexdigest()),
                )
    ...
    def load_scan(self, scan_id: str) -> ScanSnapshot:
        with self._connect() as db:
            ...
            provider_rows = db.execute(
                "SELECT payload_json FROM provider_statuses WHERE scan_id = ? ORDER BY id", (scan_id,)
            ).fetchall()
            funding_rows = db.execute(
                "SELECT payload_json FROM funding_quotes WHERE scan_id = ? ORDER BY id", (scan_id,)
            ).fetchall()
            market_rows = db.execute(
                "SELECT payload_json FROM market_quotes WHERE scan_id = ? ORDER BY id", (scan_id,)
            ).fetchall()
