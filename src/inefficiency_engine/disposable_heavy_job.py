...
async def _run(job: str) -> int:
    ...
    try:
        ...
        if job == "research":
            ...
            # is missing/stale. Runtime jobs never perform schema/index DDL because
            # DDL lock waits must not sit between the heavy-worker heartbeat and the
            # actual research/mechanism-forward cycle.
            install_bulk_provider_catalog_runtime()
            install_source_coverage_reconciliation_runtime()
            install_research_source_delegation()
