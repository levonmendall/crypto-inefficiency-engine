    ...
    while not stop_event.is_set() and (max_cycles is None or attempted < max_cycles):
        ...
        if isinstance(core_cycle, BaseException):
            ...
            if isinstance(alpha_cycle, BaseException):
                ...
                detail["alpha_forward_evidence_completed"] = True
        ...
        if isinstance(alpha_cycle, BaseException):
            ...
            detail["alpha_forward_evidence_cycle_id"] = getattr(alpha_cycle, "cycle_id", None)
            detail["alpha_candidate_count"] = getattr(alpha_cycle, "candidate_count", 0)
            detail["alpha_signals_recorded"] = getattr(alpha_cycle, "signals_recorded", 0)
            detail["alpha_outcomes_matured"] = getattr(alpha_cycle, "outcomes_matured", 0)
        if isinstance(frontier_result, BaseException):
            detail["dex_route_frontier_error_type"] = type(frontier_result).__name__
        elif frontier_result is not None:
            detail["dex_route_frontier_count"] = len(frontier_result)
