...
class _ResearchCostSettingsView:
    ...
    25-bps research assumption from hiding low-cost venue opportunities without ever
...
class ExecutableExpandedAlphaFactoryService(MemoryBoundedExpandedAlphaFactoryService):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # fast-strategy registry. Extend that registry (not the combined registry)
        # so refinements are executed instead of merely appearing in manifests.
    ...
    def _discovery_cost_floor_bps(self) -> float:
        ...
        one_way = min(usable) if usable else float(self.settings.alpha_research_cost_floor_bps) / 2.0
        return max(
            1.0,
    ...
    def _source_gate(self, candidate):
        ...
        return self.source_plane.candidate_sufficiency(
            lane_id,
            required_evidence_classes=[str(item) for item in list(LANES[lane_id]["required"])],
            primary_groups=self._primary_groups(candidate),
        )
    ...
    def discover(self, snapshot, *, total_capital_usd: float):
        ...
        for candidate in rows:
            try:
                gate = self._source_gate(candidate)
            except Exception:
                continue
            ...
            item.features.update(
                {
                    "research_cost_model": (
                        "candidate_visible_l2" if book is not None else "candidate_venue_fee_plus_fallback_slippage"
                    ),
    ...
    def qualification(self, candidate) -> AlphaQualification:
        ...
        values = [(float(row.realized_net_return), item_weight) for row, item_weight in weighted]
        mean = (
        ...
        if len(own) < local_minimum:
            blockers.append("insufficient candidate-specific forward samples for cross-asset pooling")
        ...
        effective_positive = (
            min(effective_n, int(math.floor((hit_rate or 0.0) * effective_n + 1e-9)))
            if effective_n > 0
            else 0
        )
