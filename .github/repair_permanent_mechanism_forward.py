...
# 1) Keep AllLaneEvidenceFactoryService backward-compatible, but allow the production
# disposable research factory to skip mechanism mutation now owned by the permanent
# mechanism-forward plane.
path = Path("src/inefficiency_engine/all_lane_alpha_factory.py")
text = path.read_text()
old_signature = "    async def run_evidence_cycle(self, *, total_capital_usd: float | None = None):\n        alpha = await super().run_evidence_cycle(total_capital_usd=total_capital_usd)\n"
new_signature = "    async def run_evidence_cycle(\n        self,\n        *,\n        total_capital_usd: float | None = None,\n        include_mechanism_evidence: bool = True,\n    ):\n        alpha = await super().run_evidence_cycle(total_capital_usd=total_capital_usd)\n"
if text.count(old_signature) != 1:
    raise RuntimeError("all-lane evidence cycle signature changed unexpectedly")
text = text.replace(old_signature, new_signature, 1)
method_start = text.index("    async def run_evidence_cycle(")
...
text = text[:mechanism_start] + "        if include_mechanism_evidence:\n" + indented + text[return_start:]
path.write_text(text)

# 2) Production disposable alpha research keeps discovery/funnel publication but no
# longer runs or publishes mechanism-forward mutation. The permanent worker owns it.
replace_once(
    "src/inefficiency_engine/disposable_alpha_factory.py",
    "            return await super().run_evidence_cycle(\n                total_capital_usd=total_capital_usd\n            )",
    "            return await super().run_evidence_cycle(\n                total_capital_usd=total_capital_usd,\n                include_mechanism_evidence=False,\n            )",
)
replace_once(
    "src/inefficiency_engine/disposable_alpha_factory.py",
...
# Remove this one-shot repair machinery from the final branch diff. The workflow has
# already loaded, so deleting it here does not interrupt the running job.
